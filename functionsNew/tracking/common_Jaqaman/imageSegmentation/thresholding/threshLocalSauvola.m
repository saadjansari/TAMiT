function [ imThresh, varargout ] = threshLocalSauvola( im, windowRadius, k, R, varargin )
% A fast ND implementation of sauvola's locally adaptive thresholding 
...
% 
% Typical values for k = 0.34 (between 0.2 and 0.5), R = 0.5 (between 0.2 and 1), 
% windowRadius = imageSize/8 for document images 
% 
% Usage:
% 
%   Inputs: 
...
% 
%       Typical value of k is 0.34 for document images. A suggested range
%       for trying out different values of k is [0.2,0.5]
% 
%     R:
%     
%       must be a scalar. 
%       It is used in the formula (see above) to determine the local threshold.
% 
%       Typical value of R is 0.5 for document images. A suggested range
%       for trying out different values of R is [0.2,1.0]
% 
%     padMethod (optional):
%     
...
% 
% This implementation uses integral images to speed up the computation. 
% The computational time is the same for any windowsize.
%
...
% 
    ...
    p.addRequired( 'R',  @(x) (isnumeric(x) && isscalar(x)) )
    ...
    p.parse( im, windowRadius, k, R, varargin{:} );    
    ...
    thresh_vals = meanBoxIntensity .* (1 + k * ( (stdBoxIntensity / R) - 1 ));
