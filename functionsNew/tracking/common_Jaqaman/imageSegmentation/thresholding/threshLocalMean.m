function [ imThresh, varargout ] = threshLocalMean( im, windowRadius, C, varargin )
% A fast ND implementation of locally adaptive thresholding based on local
% mean where in the threshold for a pixel (i,j) is calculated as 
% follows:
% 
%           T(i,j) = mean(i,j) - C
% 
...
% 
% Typical values for C = 10 for 8-bit document images. C = -10 seems to work well  
% for microscopic images.
% 
% Usage:
% 
%   Inputs: 
...
% 
%       Typical values for C = 10 for 8-bit document images. 
%       C = -10 seems to work well for microscopic images. 
% 
%     padMethod (optional):
%     
...
% 
    ...
    if any( windowRadius <= 0 ) || any( abs( windowRadius - round( windowRadius ) ) > 0 )   
        error( 'The window radius in each dimension should be a positive integer' );
    end
    ...
    if ~exist( 'padMethod', 'var' )
        blnPadding = false;
    else
        if ~ischar( padMethod ) || ~ismember( lower( padMethod ), { 'none', 'symmetric', 'replicate', 'circular' } )
            error( 'ERROR: padMethod must be equal to one of these strings - { none, symmetric, replicate, circular }. Check the documentation of paddarray for more details' );
        else
            blnPadding = true;
        end
    ...
    p.parse( im, windowRadius, C, varargin{:} );    
    ...
    % correct C based on the dynamic range of the image
    C = C / range(ImageIntensityRange);
    ...
    if  strcmpi( padMethod, 'none' )
        ...
        imPadded = padarray( mat2gray(im), windowRadius, padMethod ); 
    ...
    thresh_vals = meanBoxIntensity - C;
