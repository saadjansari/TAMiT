...
%parameters
parameters.linearMotion = 2; %Kalman filter for random + directed motion with the possibility of instantaneous switching to opposite direction (but same speed)
...
%optional input
parameters.diagnostics = 10; %if you want to plot the histogram of linking distances up to certain frames, indicate their numbers; 0 or empty otherwise. Does not work for the first or last frame of a movie.
...
%saveResults
saveResults.dir = 'C:\MyHome\google drive\MT analysis\line fitting'; %directory where to save input and output
saveResults.filename = 'tracksTest1.mat'; %name of file where input and output are saved
