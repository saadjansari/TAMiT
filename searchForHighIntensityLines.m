function [coords, orients] = searchForHighIntensityLines( imGauss, imLines, imMask, estParams)
    ...
    % noise removal of the steerable signal (to get rid of high frequency noise but to preserve long wavelengths along the lines)
    imEst4 = imgaussfilt( imEst, 1);
    dispImg( imEst, imEst4, [1 2])
    imEst = imEst4;
    T1 = adaptthresh( imEst, 0.01);
    imEst1 = imbinarize( imEst, T1);
    T2 = adaptthresh( imEst, 0.03);
    imEst2 = imbinarize( imEst, T2);
    T3 = adaptthresh( imEst, 0.05);
    imEst3 = imbinarize( imEst, T3);
    dispImg(imEst, imEst1, imEst2, imEst3, [2 2]);
