function [bestk,bestpp,bestmu,bestcov,dl,countf] = mixtures4plus(y,kmax,covar,covoption,varargin)
%
% Syntax:
...
% covoption = 4 means a fixed, common covariance for all components
%
...
% "cov" initial guess of covariance matrix (see covoption above for details)
...
%   2000, 2001, 2002
% 
% edited & extended by US, 2013/01/29
%
...
ip.addRequired('kmax',@isscalar);
ip.addRequired('covar',@isnumeric);
ip.addRequired('covoption',@isscalar);

% optional arguments
ip.addOptional('kmin',1,@isscalar);
ip.addOptional('regularize',0.00001,@isscalar);
ip.addOptional('th',1e-6,@isscalar);
ip.addOptional('mu',[],@isnumeric);
ip.addOptional('pp',[],@isnumeric);
ip.addOptional('verbose',[],@isnumeric);

% parse arguments
ip.parse(y,kmax,covar,covoption,varargin{:});

y=ip.Results.y;
kmax=ip.Results.kmax;
covoption=ip.Results.covoption;

[dimens,npoints] = size(y);

if covoption == 4
    covar=ip.Results.covar;
else
    ...
    for i=1:kmax
        ...
        covar(:,:,i) = diag(ones(1,dimens)*max(diag(globcov/10)));
    end
end

kmin=ip.Results.kmin;
regularize=ip.Results.regularize;
th=ip.Results.th;
mu=ip.Results.mu;
pp=ip.Results.pp;
...
mismatchError=0;
if ~isempty(mu) || ~isempty(pp)
    [nr,nc]=size(mu);
    if nc ~= kmax || nr ~= dimens
        mismatchError=1;
    end
...
nr=size(covar,2);
nk=size(covar,3);
if nc ~= dimens || nr ~= dimens || nk ~= kmax
    mismatchError=1;
end

if mismatchError
    fprintf('value of kmax: %ld\n',kmax);
    fprintf('dimension: %ld\n',dimens);
    if ~isempty(mu)
        fprintf('size of mu: [%ld,%ld]\n',size(mu,1),size(mu,2));
    end
    if ~isempty(pp)
        fprintf('size of pp: [%ld,%ld]\n',size(pp,1),size(pp,2));
    end
    fprintf('size of covar: [%ld,%ld,%ld]\n',size(covar,1),size(covar,2),size(covar,3));
    error('Mismatch between supplied values');
end
...
switch covoption
    ...
    case 3 
        npars = dimens;
    % case 4 and 5 are for fixed covariances
    case 4
        npars = dimens;
    case 5
        npars = dimens;
