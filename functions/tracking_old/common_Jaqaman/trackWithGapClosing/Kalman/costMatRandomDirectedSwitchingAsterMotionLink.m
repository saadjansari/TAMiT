function [costMat,propagationScheme,kalmanFilterInfoFrame2,nonlinkMarker,...
    errFlag] = costMatRandomDirectedSwitchingAsterMotionLink(movieInfo,kalmanFilterInfoFrame1,...
...
%             .allCoord           : L,dL,theta,dtheta,[phi,dphi] of features collected in one
%                                   matrix.
...
%                                   1 for random + directed motion of length;
%                                   2 for random + directed motion of length with the
...
if nargin ~= nargin('costMatRandomDirectedAsterSwitchingMotionLink')
    disp('--costMatRandomDirectedSwitchingAsterMotionLink: Incorrect number of input arguments!');
    errFlag  = 1;
    return
end
...
switch linearMotion
    ...
    case 1 %random motion + directed motion
        diagVec=zeros(probDim,1);
        diagVec(1)=1;
        transMat(:,:,1) = eye(vecSize) + diag(diagVec,probDim); 
        %forward drift transition matrix in length, angles remain random
    ...
    case 2 %random motion + directed motion that can switch to opposite direction at any moment
        ...
        transMat(:,:,1) = eye(vecSize) + diag(diagVec,probDim);
        %forward drift transition matrix in length, angles remain random
        transMat(:,:,2) = eye(vecSize) + diag(-diagVec,probDim);
        %reverse drift transition matrix in length, angles remain random
...
for iScheme = 1 : numSchemes
    ...
    costMatTmp(:,:,iScheme) = createDistanceMatrixPolar(coord1,coord2);
