...
%                     error('not set up')
        ...
        function temp_fig1( obj, jChannel)
            ...
            % 3D FIGURE
            if strcmp(obj.cellType, 'MitosisBud')
                view_angle = [-17.8,18.9];
                zlimits = [5,11];
            elseif strcmp(obj.cellType, 'Mitosis')
                view_angle = [-11.6,21.2];
                zlimits = [4,8];
            elseif strcmp(obj.cellType, 'Monopolar')
                view_angle = [-12.8,12.4];
                zlimits = [1,9];
            end
...
%             set(gca,'Position', [pos(1), pos(2), pos(3), pos(3)*size(img,1)/size(img,2)]);
            ...
            % Plot Shadow on lower zlimit XY plane
            % DUplicate feature
            featShadow = feat.copyDeep();
            z_shadow = zlimits(1);
            if strcmp(obj.cellType, 'MitosisBud')
                featShadow.featureList{1}.startPosition(3) = z_shadow;
                featShadow.featureList{1}.endPosition(3) = z_shadow;
                for ja = 2: featShadow.numFeatures
                    featShadow.featureList{ja}.featureList{1}.position(3) = z_shadow;
                    for jf = 2: featShadow.featureList{ja}.numFeatures
                        featShadow.featureList{ja}.featureList{jf}.origin(3) = z_shadow;
                        featShadow.featureList{ja}.featureList{jf}.thetaInit(2) = pi/2;
                    end
                end
            elseif strcmp(obj.cellType, 'Mitosis')
                featShadow.featureList{1}.startPosition(3) = z_shadow;
                ...
                for ja = 2: featShadow.numFeatures
                    featShadow.featureList{ja}.featureList{1}.position(3) = z_shadow;
                    for jf = 2: featShadow.featureList{ja}.numFeatures
                        featShadow.featureList{ja}.featureList{jf}.startPosition(3) = z_shadow;
                        featShadow.featureList{ja}.featureList{jf}.endPosition(3) = z_shadow;
                    end
                end
            elseif strcmp(obj.cellType, 'Monopolar')
                ja=1;
            ...
            featShadow.displayFeature3D( gca, 1.25*z_shadow, gray)
