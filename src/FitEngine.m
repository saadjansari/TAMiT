...
%             ms = MultiStart('PlotFcns',@gsplotbestf, 'StartPointsToRun','bounds', 'Display', 'iter');
        ...
        function [obj,fitInfo] = zpadding_on( obj, fitInfo, pad_xy, pad_z)
            ...
            img1(1+pad_xy:end-pad_xy, 1+pad_xy:end-pad_xy, 1) = median( img0( img0(:) ~= 0) );
            img1(1+pad_xy:end-pad_xy, 1+pad_xy:end-pad_xy, end) = median( img0( img0(:) ~= 0) );
            obj.image = img1;
...
%                     vargout = p2;
                    ...
                    FF = ((resR1-resR2)/(dof1-dof2)) / (resR2/dof2);
                    pp = 1-fcdf(FF,dof1-dof2,dof2);
                    vargout = pp;
